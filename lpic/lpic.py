...
def write_solutions( path, questions, versio=None):

    path += "_Respostes"
    if versio is not None :
        path += "_" + V
    path += ".txt"
    ...
    for i in range(len(questions)) :
        ...
        if questions[i].solucio is not None :
            f.write(str(chr(questions[i].solucio + ord('a'))))
    f.close()
...
def generate_pdf( path, questions, versio=None, explicacions=False, marcar_solucio=False ):

    path = path.split("/")
    file_name = path[-1]
    if marcar_solucio :
        file_name += "_Solucions"
    if versio is not None :
        file_name += "_" + versio
    file_name += ".pdf"
    path = path[:-1] + [file_name]
    path = "/".join(path)
    ...
    pdf.write("Nom i cognoms:", w=30, h=6, border=1)
    pdf.line_up(h=6)
    pdf.write("",h=6,border=1)
    pdf.write("Data:", w=30, h=6, border=1)
    pdf.line_up(h=6)
    pdf.write("",h=6,border=1)

    pdf.move_pos(y=2.5)
    pdf.set_font("Arial", style="B", size=10)
    pdf.write("TEST LPIC - UNITAT FORMATIVA 2 - VERSIO {}".format(versio), align="L")
    pdf.move_pos(y=2)
    pdf.set_font("Arial", style="", size=10)
    pdf.write("Encercleu la resposta/es que considereu correctes. " + 
              "Si us equivoqueu en marcar-la, tatxeu-la amb una creu. " +
              "Totes les preguntes compten igual i 4 respostes incorrectes resten una correcta. " +
              "Les preguntes no contestades no resten. Les preguntes 26 a 30 no resten si es contesten incorrectament.", align="J")
    ...
    for question in questions :
        ...
        if same_space :
            ...
            if question.solucio is None :
                pdf.write_white( h=20 )
            else :
...
for i in range(2):
    V = chr( ord( 'A' ) + i )
    new_questions = shuffle ( questions, threshold=-5 )
    generate_pdf( output + "ExamenLPIC_UF2-1819", new_questions, V,  explicacions=False, marcar_solucio=False)
    generate_pdf( output + "ExamenLPIC_UF2-1819", new_questions, V,  explicacions=False, marcar_solucio=True)
    write_solutions( output + "ExamenLPIC_UF2-1819", new_questions, V )
